...
from solutions.day12 import Day12PartA, Moon
...
class TestDay12PartA:
    ...
    def test_creation(self):
        ...
        input_string = "\n".join(input_strings)
        solution = Day12PartA()
        solution.create_moons(input_string)

        assert len(solution.moons) == 4
        assert (
            repr(solution.moons[0])
        ...
        for i, expected_position in enumerate(step_1_pos):
            assert repr(solution.moons[i]) == expected_position

        # Step another 9 times to get to step 10:
        solution.step_multi(9)
        step_10_pos = [
        ...
        assert solution.get_total_energy() == 179
